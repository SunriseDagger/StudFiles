...
int grid_gen_cube::generate_reg_grid_pr(gen_point left_bottom, gen_point right_top, double h_xmin, double h_ymin, double h_zmin, gen_point *&grid_mass){
	...
	//Генерация одномерных сеток по переменным
	grid_gen::generate_reg_grid(a_x, b_x, h_xmin, n_x, mass_x);
	grid_gen::generate_reg_grid(a_y, b_y, h_ymin, n_y, mass_y);
	grid_gen::generate_reg_grid(a_z, b_z, h_zmin, n_z, mass_z);
...
bool grid_gen_cube::generate_reg_grid(gen_point left_bottom, gen_point right_top, double h_xmin, double h_ymin, double h_zmin, std::string file_name){
	//Проверка коректности данных, получение необходиммых данных
	//Координаты для начал отрезков генерации по перменным
	double a_x = left_bottom.x;
	double a_y = left_bottom.y;
	...
	gen_point* grid_mass; //сюда сетку
	int n = generate_reg_grid_pr(left_bottom, right_top, h_xmin, h_ymin, h_zmin, grid_mass);
...
//И самое весёлое - формирование и вывод элементов, часть там копипаст, кстати.
bool grid_gen_cube::generate_reg_grid_FEM(gen_point left_bottom, gen_point right_top, double h_xmin, double h_ymin, double h_zmin, string file_cords, string file_elements, string file_faces){

	if(!generate_reg_grid(left_bottom, right_top, h_xmin, h_ymin, h_zmin, file_cords)) return false; //Вывод точек и проверка на корректность за одно
	...
	int n_x = grid_gen::nodes_number_gen_reg(a_x, b_x, h_xmin);
	int n_y = grid_gen::nodes_number_gen_reg(a_y, b_y, h_ymin);
	int n_z = grid_gen::nodes_number_gen_reg(a_z, b_z, h_zmin);
	...
	for(int i = 0; i < n_x - 1; i++){
		for(int j = 0; j < n_y - 1; j++){
			for(int k = 0; k < n_z - 1; k++){
				int node_i0 = k*n_xy + j*n_x + i; //глобальный номер вершины
				//Остальные 7 вершин
				//Те, который в той же плоскти по xy
				int node_i1 = k*n_xy + j*n_x + i+1;
				int node_i2 = k*n_xy + (j+1)*n_x + i;
				int node_i3 = k*n_xy + (j+1)*n_x + i + 1;

				//Те, которые выше
				int node_i4 = (k+1)*n_xy + j*n_x + i;
				int node_i5 = (k+1)*n_xy + j*n_x + i+1;
				int node_i6 = (k+1)*n_xy + (j+1)*n_x + i;
				int node_i7 = (k+1)*n_xy + (j+1)*n_x + i + 1;
