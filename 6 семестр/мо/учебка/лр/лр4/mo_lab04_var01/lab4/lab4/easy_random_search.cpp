...
//	double fk; //значение функции в точек xk
	...
	vector<vect_2d> points; //генерируемые точки
	vector<double> vals; //значени€ функции
...
#pragma omp for private(points, vals, num)
	for(p_iter = 0; p_iter < ch_n; p_iter++){
		num = omp_get_thread_num(); //узнаЄм номер потока
