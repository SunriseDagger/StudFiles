...
{
    if(chain == -1) return false;
    int dimension_real = (1 > dimension ? 1 : dimension);
    table[hash][chain].dimension = dimension;
    table[hash][chain].is_init.resize(dimension_real);
    for(int i = 0; i < dimension_real; i++)
    ...
    for(int i = 0; i < hashnum; i++)
    {
        if(table[i].size())
        {
            ...
            for(int j = 0; j < (int)table[i].size(); j++)
            {
                ...
                for(int k = 0; k < table[i][j].dimension || k < 1; k++)
                {
                    stream << table[i][j].is_init[k];
                    if(k != table[i][j].dimension - 1 && table[i][j].dimension != 0) stream << " ";
                }
